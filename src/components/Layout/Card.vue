		...
		<div class="mb-4">
			...
			>
				...
				>
					...
					<div v-if="!regex.test(item.data[0])" class="px-2">
						<ul class="text-xs">
							<li v-for="(value, idx) in item.data" :key="idx">
								{{ value }}
							</li>
						</ul>
					</div>
					...
					<div
						v-else
						class="px-2 flex flex-wrap flex-col max-h-[400px] gap-x-6"
					>
						<p v-for="(value, idx) in item.data" :key="idx" class="text-xs">
							{{ value }}
						</p>
					</div>
				</div>
