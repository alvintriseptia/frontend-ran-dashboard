<template>
	<section>
		<el-table
			...
			ref="activityTable"
		>
			<el-table-column label="No" width="60" column-key="no">
...
const positionActivityTable = ref({
	x: 0,
});
const window = useWindow();

onMounted(() => {
	const children = activityTable.value.$children;
	let position = 0;

	for (let i = 0; i < children.length; i++) {
		if (children[i].columnKey) {
			if (children[i].columnKey === "namaPIC") {
				break;
			} else {
				position += children[i].realWidth;
			}
		}
	}

	positionActivityTable.value = {
		x: position,
	};

	console.log(positionActivityTable.value.x);
});
